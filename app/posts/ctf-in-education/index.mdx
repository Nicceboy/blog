...
Of course, we could force students to take exams in controlled environment, but then we need to review the tasks and the evaluation is based more about knowledge retention, rather than students using their computers to solve problems in a creative way.
...
We should definitely seek new approaches to technical teaching in cybersecurity, as we can no longer use knowledge retention as main measurement of skill.
We coud force students to take traditional exams in supervised classrooms, but maybe that should be only a minor aspect, and only for the knowledge that must be remembered by students itself.
What that information is - it is difficult to determine.
If students have access to the LLMs in their job, we should teach their problem solving skills and thinking so that they can get most benefits from them.
